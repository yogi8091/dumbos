...
app.controller('appController', function($scope, appFactory){
	...
	$scope.queryAllDrug = function(){
        ...
        $scope.queryHistDrug = function(){

                var id1 = $scope.hist_drug_id;

                appFactory.queryHistDrug(id1, function(data){
                        var array = [];
                        for (var i = 0; i < data.length; i++){
                                data[i].Record.Key = id1;
                                array.push(data[i].Record);
                        }
                        $scope.hist_drug = array;
                });
        }
...
app.factory('appFactory', function($http){
    ...
    factory.queryAllDrug = function(callback){
        ...
        factory.queryHistDrug = function(id, callback){
        $http.get('/get_hist_drug/'+id).success(function(output){
                        callback(output)
                });
        }
